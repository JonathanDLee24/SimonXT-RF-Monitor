...
Current Bugs are that is seldomly will log ghost events and flip the logic can be fixed by increasing the timer on if (liveCache[cacheIdx].pendingConfirmCount >= 3) to 3 in place of 2 again this causes issues with the actaul events it will seldomly miss them. 
